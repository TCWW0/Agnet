...
// 字节流喂进一个会记账的模型里。
//
// 刻意只实现本项目真正发出的那几个序列：CUP、EL(0)、ED(0)、DECAWM 的 ?7h/?7l、
// 备用屏 ?1049h/l、光标 ?25h/l。遇到没实现的序列会记进 unhandled() —— 静默忽略
// 会让探针在实现变化后悄悄失去鉴别力，而那正是「断言通过但没有鉴别力」最隐蔽的
// 一种。
//
...
// 字符串模型都表达不了。刻意不针对「frame_bytes 总是从第 1 列开始整行重写」
// 这个当前事实做简化 —— 那样一旦实现改动，探针会悄悄不再鉴别。
